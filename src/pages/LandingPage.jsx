...
const FEATURES = [
  {
    ...
    desc: "Upload any image of a cat or dog and get an instant prediction with confidence score. Powered by MobileNetV2 transfer learning trained on 1,000+ pet images.",
  },
  {
    icon: "⚡",
...
const FLOW_STEPS = [
  { step: "01", label: "Input",        desc: "User uploads a pet image or captures one via webcam in the React app" },
  { step: "02", label: "Transfer",     desc: "The image is sent as a multipart form request to the FastAPI server on port 8000" },
  { step: "03", label: "Preprocessing",desc: "The server resizes the image to 160×160 px and normalizes pixel values to [0, 1]" },
  { step: "04", label: "Inference",    desc: "MobileNetV2 model runs a forward pass and outputs a softmax probability for each class" },
  { step: "05", label: "Prediction",   desc: "The predicted label (Cat / Dog) and confidence % are returned as JSON to the React app" },
];
    ...
    <div className="min-h-full bg-slate-950 text-white">
      ...
      <section className="relative px-6 pt-24 pb-20 text-center overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-[350px] bg-orange-600/15 rounded-full blur-3xl" />
        </div>

        <div className="relative max-w-3xl mx-auto">
          <span className="inline-block text-xs font-semibold tracking-widest text-orange-400 uppercase mb-5 px-3 py-1 rounded-full border border-orange-500/30 bg-orange-500/10">
            ITE03 + EVENTDP · Finals Project
          </span>

          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black mb-4 leading-tight whitespace-nowrap">
            🐾 Cats and Dogs Classifier
          </h1>

          <p className="text-xl text-slate-400 mb-10">
            AI-powered pet classifier — upload any image and instantly know if it's a cat or a dog.
          </p>

          <div className="flex flex-wrap justify-center gap-3">
            <Link
              to="/classify-pets"
              className="bg-orange-600 hover:bg-orange-500 text-white px-8 py-3 rounded-xl font-semibold transition-all shadow-lg shadow-orange-900/40 hover:-translate-y-0.5"
            >
              🐾 Try the Classifier →
            </Link>
            <Link
      ...
      <section className="max-w-5xl mx-auto px-6 pb-20">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-7">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-xl bg-orange-600/20 border border-orange-500/30 flex items-center justify-center text-base">🐾</div>
              <span className="text-xs font-semibold text-orange-400 uppercase tracking-wide">EVENTDP Component</span>
            </div>
            <p className="text-slate-400 text-sm leading-relaxed mb-3">
              The <span className="text-white font-medium">Cats and Dogs Classifier</span> is a deep learning
              image classification system built with
              <span className="text-orange-400 font-medium"> MobileNetV2 transfer learning</span> — a technique
              where a model pre-trained on ImageNet is fine-tuned for a specific task.
            </p>
            <p className="text-slate-400 text-sm leading-relaxed">
              The model was trained on the Kaggle Dogs and Cats dataset achieving
              <span className="text-orange-400 font-medium"> 91.08% validation accuracy</span> and
              <span className="text-orange-400 font-medium"> 90.17% test accuracy</span> using two-phase
              training — first the classification head, then fine-tuning the top MobileNetV2 layers.
              Served through a FastAPI Python backend on port 8000.
            </p>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-7">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-3">
              ...
              web application for managing student enrollment records. Secured with
        ...
        <div className="flex flex-wrap gap-3 mt-5">
          <div className="flex items-center gap-2 bg-orange-600/10 border border-orange-500/20 rounded-xl px-4 py-2">
            <span className="w-2 h-2 rounded-full bg-orange-500" />
            <span className="text-xs font-semibold text-orange-400">EVENTDP — Event-Driven Programming</span>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-10 bg-slate-900 border border-slate-800 rounded-2xl p-5">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 text-sm font-semibold">
              {["Upload Pet Image", "→", "FastAPI Receives File", "→", "Preprocessing", "→", "MobileNetV2 Inference", "→", "JSON Result", "→", "React Displays Prediction"].map((item, i) => (
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="bg-slate-800 px-4 py-2.5 flex items-center gap-2 border-b border-slate-700">
                ...
                <div className="flex-1 bg-slate-700 rounded-md h-5 mx-2 flex items-center px-2">
                  <span className="text-xs text-slate-500">localhost:5173/classify-pets</span>
                </div>
              </div>
              <div className="p-6">
                <p className="text-xs text-slate-500 uppercase tracking-widest mb-3">Pets Classifier</p>
                <div className="border-2 border-dashed border-slate-700 rounded-xl p-6 text-center mb-4">
                  <div className="text-2xl mb-2">�</div>
                  <p className="text-xs text-slate-500">Click or drag &amp; drop a pet image</p>
                </div>
                <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4 text-center">
                  <p className="text-xs text-slate-500 mb-1">Prediction</p>
                  <p className="text-2xl font-black text-white">🐱 Cat</p>
                  <div className="mt-2 bg-slate-800 rounded-full h-2">
                    <div className="bg-orange-500 h-2 rounded-full w-[91%]" />
                  </div>
                  <p className="text-xs text-slate-400 mt-1">91.08%</p>
                </div>
              </div>
              <p className="text-center text-xs text-slate-600 pb-4">Cats and Dogs Classification</p>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="bg-slate-800 px-4 py-2.5 flex items-center gap-2 border-b border-slate-700">
                ...
                <div className="flex-1 bg-slate-700 rounded-md h-5 mx-2 flex items-center px-2">
                  <span className="text-xs text-slate-500">localhost:5173/students</span>
                </div>
              </div>
              <div className="p-6">
          ...
          <div className="mt-5 bg-slate-900 border border-slate-800 rounded-2xl p-7">
            ...
            <div className="flex flex-wrap items-stretch justify-center gap-3">
              {[
                { label: "Pet Image",            sub: "JPG / PNG / WEBP",                          color: "border-slate-700 bg-slate-800" },
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-800">
              <span className="text-xs text-slate-500">Course / Year:</span>
              <span className="text-xs font-semibold text-orange-400 bg-orange-500/10 border border-orange-500/20 px-3 py-1 rounded-full">BSIT-III</span>
              <span className="text-xs font-semibold text-orange-400 bg-orange-500/10 border border-orange-500/20 px-3 py-1 rounded-full">BSCS-III</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm text-slate-400">Cats and Dogs Classifier</span>
            </div>
          ...
          <div className="border-t border-slate-800 mt-6 pt-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-600">
            <p>© 2026 Cats and Dogs Classifier — ITE03 + EVENTDP Final Project</p>
            <p>Almodovar · Marinas · Serrano · Dela Pena · BSIT-III / BSCS-III</p>
          </div>
        </div>
      </footer>
...
function SectionLabel({ children }) {
  return (
    <span className="inline-block text-xs font-semibold tracking-widest text-orange-400 uppercase mb-3 px-3 py-1 rounded-full border border-orange-500/30 bg-orange-500/10">
      {children}
    </span>
  );
}
